...
function updatePlatformOpenTargetButton(url='',info=null){
  const btn=$('platformOpenTargetBtn');if(!btn)return;
  const safe=cleanMobileTargetUrl(url);
  btn.hidden=!safe;
  btn.dataset.targetUrl=safe||'';
  btn.textContent=safe?'前往 '+(info?.shortName||info?.name||'目標網站'):'前往目標網站';
}
async function detectPlatformFromInput({adopt=true}={}){
  ...
  if(!raw.trim()){updatePlatformOpenTargetButton();$('platformStatus').textContent='貼上售票或購物網址，QuickBuy 會自動辨識。';return null;}
  if(!safe){updatePlatformOpenTargetButton();$('platformStatus').textContent='沒有讀到有效網址；可以直接貼整段分享文字，QuickBuy 會自動抓出連結。';return null;}
  if(!info){
    updatePlatformOpenTargetButton(safe,null);
  ...
  updatePlatformOpenTargetButton(safe,info);
...
async function openShortcut(id){const x=customShortcuts.find(v=>v.id===id);if(!x)return;const preopened=preopenPwaTarget();try{x.lastUsedAt=Date.now();await saveCustomShortcuts();$('platformUrl').value=x.url;detectPlatformFromInput();if(globalThis.QBA_PWA){const p=globalThis.QBA_PLATFORM_CATALOG?.detect?.(x.url)||null;mobilePrep.platformId=p?.id||'';mobilePrep.targetUrl=cleanMobileTargetUrl(x.url);mobilePrep.updatedAt=Date.now();try{await saveMobilePrep('',{quiet:true});}catch(_){}renderMobileJourneyState();}await navigateSafari(x.url,preopened);mobileToast(`已開啟 ${x.name}`,'good',1000);if(!globalThis.QBA_PWA)setTimeout(()=>{try{window.close();}catch(_){}},80);}catch(_){try{preopened?.close?.();}catch(__){}mobileToast(globalThis.QBA_PWA?'已選收藏，可按前往繼續':'已選收藏，請回 Safari 繼續','good');}}
...
function renderMobileJourneyState(current=null){const wrap=$('mobileJourneyState'),title=$('mobileJourneyTitle'),detail=$('mobileJourneyDetail');if(!wrap||!title||!detail)return;let cls='journey',t='還差一步',d=globalThis.QBA_PWA?'先選平台，再設定數量或規格。':'先在 Safari 打開售票或商品頁。';const target=mobilePrep.targetUrl||'',p=platformById(mobilePrep.platformId);const swapTargetBtn=$('mobileSwapTargetBtn'),clearTargetBtn=$('mobileClearTargetBtn'),reopenTargetBtn=$('mobileReopenTargetBtn');if(swapTargetBtn)swapTargetBtn.textContent=target?'✎ 換網址':'⌘ 貼網址';if(clearTargetBtn)clearTargetBtn.hidden=!target;if(reopenTargetBtn)reopenTargetBtn.hidden=!target;const raw=$('mobileTargetAt')?.value||mobilePrep.targetAt||'';const targetMs=raw?new Date(raw).getTime():NaN;const now=Date.now();if(target){cls+=' ready';t='準備完成';d=`${p?.shortName||p?.name||'目前網站'} · 數量 ${mobilePrep.quantity}${mobilePrep.keyword?` · ${mobilePrep.keyword}`:''}`;if(Number.isFinite(targetMs)&&targetMs>now){cls='journey wait';t='已準備・等待開賣';const mins=Math.ceil((targetMs-now)/60000);d=mins<=1?'不到 1 分鐘，保持 Safari 頁面準備好。':`約 ${mins} 分鐘後，時間到再回 Safari 確認。`;}else if(Number.isFinite(targetMs)&&targetMs<=now){cls='journey now';t='時間到了';d='回 Safari 確認頁面狀態；需要登入或驗證時由你完成。';}}
...
wrap.className=cls;title.textContent=t;detail.textContent=d;const useBtn=$('mobileUseCurrentBtn'),sameCurrent=Boolean(current?.safeUrl&&target&&current.safeUrl===target);if(useBtn){useBtn.hidden=Boolean(globalThis.QBA_PWA);useBtn.classList.toggle('ready-action',sameCurrent);useBtn.textContent=globalThis.QBA_PWA?(sameCurrent?'目標已準備':'使用目前目標'):(sameCurrent?'重新讀取 Safari':'重新讀取目前頁面');}renderMobileReadiness(current);}
...
$('mobileWaitReturnBtn')?.addEventListener('click',async()=>{const preopened=preopenPwaTarget();try{await saveMobilePrep('',{quiet:true});}catch(_){}if(globalThis.QBA_PWA){if(!mobilePrep.targetUrl){try{preopened?.close?.();}catch(_){}showMobileView('platforms');return;}try{await navigateSafari(mobilePrep.targetUrl,preopened);}catch(e){try{preopened?.close?.();}catch(_){}mobileToast(humanMobileError(e,'無法開啟目標'),'error',2400);}return;}try{window.close();}catch(_){mobileToast('可以直接切回 Safari 繼續','good');}});
