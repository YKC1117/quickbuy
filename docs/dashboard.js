...
function sanitizeShortcutUrl(raw){try{const u=new URL(String(raw||'').trim());if(!/^https?:$/.test(u.protocol))return'';u.username='';u.password='';u.search='';u.hash='';return `${u.origin}${u.pathname||'/'}`;}catch(_){return'';}}
...
function cleanMobileTargetUrl(raw){try{const u=new URL(String(raw||'').trim());if(!/^https?:$/.test(u.protocol))return'';u.username='';u.password='';u.search='';u.hash='';return `${u.origin}${u.pathname||'/'}`;}catch(_){return'';}}
...
function mobileToast(message,tone='good',ttl=1800){const el=$('mobileToast');if(!el)return;clearTimeout(mobileToastTimer);el.className=`mobile-toast ${tone} show`;el.textContent=String(message||'');mobileToastTimer=setTimeout(()=>el.classList.remove('show'),ttl);}
...
async function adoptCurrentPage({manual=false,allowRecognizedAuto=true,reason='foreground'}={}){const current=await refreshMobileCurrentPage();if(!current.safeUrl){renderMobileJourneyState(current);if(manual){const msg=current.restricted?'QuickBuy 目前看不到這個 Safari 網站，請先確認網站存取權限。':'請先在 Safari 打開售票、商品或活動頁。';setMobileHomeStatus(msg,'warn');mobileToast(current.restricted?'先確認 Safari 網站權限':'先到 Safari 打開你要的頁面','warn');}return{...current,adopted:false};}
  ...
  const changed=mobilePrep.targetUrl!==current.safeUrl||mobilePrep.platformId!==(currentPlatform?.id||'');mobilePrep.platformId=currentPlatform?.id||'';mobilePrep.targetUrl=current.safeUrl;await saveMobilePrep('',{quiet:true});
